...
> 임베딩과 Chroma 벡터DB, BM25 하이브리드 검색을 사용한다. 응답은 검색된 원문 snippet과
> 결정론 키워드 신호만 근거로 하며, 법적 적합성은 단정하지 않는다.
...
| `check_disclosure_risk(text)` | ✅ **실제** | 로컬 RAG 근거와 키워드 신호로 대외공유/공시 위험을 보수적으로 안내 | — |
| `search_compliance_rule(query)` | ✅ **실제** | 로컬 규정 코퍼스에서 Chroma + BM25 하이브리드 검색으로 근거 snippet 반환 | — |
...
`bge-m3`는 RAG 임베딩 전용 모델이다. MCP 서버는 외부 임베딩 API를 쓰지 않고
`http://localhost:11434/api/embed`의 로컬 Ollama 엔드포인트만 호출한다.
