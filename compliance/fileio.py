...
def _read_pdf(file_path: str) -> tuple[str, int]:
    try:
        ...
        raise FileInputError(f"PDF를 열 수 없습니다(손상되었거나 암호화됨): {exc}") from exc
    if reader.is_encrypted:
        raise FileInputError("암호화된 PDF는 지원하지 않습니다. 암호를 해제한 사본으로 다시 시도하세요.")
    parts: list[str] = []
...
def extract_text(file_path: str) -> ExtractedText:
    ...
    if size > MAX_FILE_BYTES:
        raise FileInputError(f"파일이 너무 큽니다({size} bytes, 상한 {MAX_FILE_BYTES}).")
